...
int check_run_length(int *msg, int size, int *code) {
    /****** To be completed ******/
    int equal = 1;
    int *temp = (int *) malloc(sizeof(int) * MAX_MSG_SIZE);
    int new_size = run_length_decode(code, size, temp);
